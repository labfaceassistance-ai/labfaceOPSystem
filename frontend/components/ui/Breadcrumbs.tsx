                ...
                {breadcrumbs.map((breadcrumb, index) => (
                    <li key={breadcrumb.href} className="flex items-center space-x-2">
                        ...
                        {index === breadcrumbs.length - 1 ? (
                            <span className="text-[10px] font-black uppercase text-identity-sky bg-identity-sky/5 px-3 py-1 rounded-full border border-identity-sky/10">
                                {breadcrumb.label}
                            </span>
                        ) : (
                            <Link 
                                href={breadcrumb.href}
                                className="text-[10px] font-black uppercase text-identity-navy/40 hover:text-identity-navy transition-colors px-2 py-1"
                            >
                                {breadcrumb.label}
                            </Link>
                        )}
