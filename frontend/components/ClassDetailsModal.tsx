...
import { X, Calendar, Clock, CheckCircle, XCircle, AlertCircle, FileText, Edit2, Save, RotateCcw, Download, Users, History, Camera, Upload, Ban, ChevronRight, ArrowRight, ShieldCheck, Activity } from 'lucide-react';
...
export default function ClassDetailsModal({ isOpen, onClose, classId, className, initialView = 'list', isArchived }: ClassDetailsModalProps) {
    ...
    const filteredSessions = sessions
        ...
        .sort((a, b) => {
            const timeA = new Date(`${a.date.split('T')[0]}T${a.startTime}`).getTime();
            const timeB = new Date(`${b.date.split('T')[0]}T${b.startTime}`).getTime();
            return timeA - timeB;
        });
    ...
    return (
        <div className="fixed inset-0 bg-[#041C3C]/40 backdrop-blur-2xl flex items-center justify-center z-50 animate-in fade-in duration-500 p-4 font-outfit">
            <div className="bg-white/40 backdrop-blur-3xl w-full max-w-[96vw] h-[92vh] rounded-[4rem] border border-white/20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-500 relative">
                ...
                {/* Header */}
                <div className="flex flex-col md:flex-row justify-between items-center p-10 md:p-14 border-b border-white/20 bg-gradient-to-b from-[#041C3C]/5 to-transparent relative z-10 shrink-0 gap-10">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic opacity-70">
                                    CENTRAL_METRIC_MODULE: {viewMode === 'list' ? 'COHORT_PROFILES' : 'CHRONOLOGICAL_LOGS'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <div className="bg-white/60 backdrop-blur-md p-2 rounded-[2rem] border border-white/50 flex gap-1 shadow-inner">
                            <button
                                onClick={() => setViewMode('list')}
                                className={`px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 flex items-center gap-3 ${viewMode === 'list' ? 'bg-[#041C3C] text-white shadow-xl' : 'text-slate-400 hover:text-identity-navy hover:bg-white/80'}`}
                            >
                                <Users size={16} /> COHORT
                            </button>
                            <button
                                onClick={() => setViewMode('history')}
                                className={`px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 flex items-center gap-3 ${viewMode === 'history' ? 'bg-[#041C3C] text-white shadow-xl' : 'text-slate-400 hover:text-identity-navy hover:bg-white/80'}`}
                            >
                        ...
                        {viewMode === 'history' && (
                            ...
                            >
                                <Download size={18} className="group-hover:translate-y-1 transition-transform" /> EXPORT_DATASET
                            </button>
                        )}
                        ...
                        {!isEditing ? (
                            !isArchived && viewMode === 'history' && (
                                ...
                                >
                                    <Edit2 size={18} className="group-hover:rotate-12 transition-transform" /> MODIFY_LOGS
                                </button>
                            )
                        ) : (
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center text-center space-y-8">
                            ...
                            <div className="space-y-3">
                                <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic leading-none">Loading Data...</h3>
                                <p className="text-[10px] font-black text-[#5CB4E4] uppercase tracking-[0.4em] italic opacity-60">Synchronizing Class Records</p>
                            </div>
                        </div>
                    </div>
                    ) : (
                        <div className="flex flex-col h-full min-w-0">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
                                {[
                                    { label: 'COHORT_NODES', value: students.length, suffix: 'SUBSCRIBERS', icon: Users, color: 'text-[#041C3C]', bg: 'bg-[#041C3C]/5' },
                                    { label: 'LOG_ENTRIES', value: sessions.length, suffix: 'CHANNELS', icon: Activity, color: 'text-[#5CB4E4]', bg: 'bg-[#5CB4E4]/5' },
                                    { 
                                        label: 'ENGAGEMENT_VELOCITY', 
                                        value: sessions.length > 0 ? Math.round((students.reduce((acc, s) => acc + s.attendance.filter((a: any) => a.status === 'Present' || a.status === 'Late').length, 0) / (students.length * sessions.length)) * 100) : 0, 
                                        suffix: '', 
                                        icon: ShieldCheck, 
                                    ...
                                    <div key={i} className={`${stat.bg} p-5 rounded-[2rem] border border-slate-100 relative group overflow-hidden transition-all duration-500 hover:scale-[1.02] shadow-sm`}>
                                        <div className="relative z-10 flex flex-col justify-between h-full">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className={`p-3 rounded-xl bg-white shadow-lg ${stat.color}`}>
                                                    <stat.icon size={20} />
                                                </div>
                                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">{stat.label}</span>
                                            </div>
                                            <div className="flex items-baseline gap-2">
                                                <span className={`text-3xl font-black ${stat.color} italic font-outfit`}>{stat.value}</span>
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{stat.suffix}</span>
                                            </div>
                                        </div>
                            ...
                            <div className={`flex-1 flex flex-col min-h-0 bg-white/60 backdrop-blur-md rounded-[3.5rem] border border-white/50 shadow-inner overflow-hidden ${isEditing ? 'ring-8 ring-[#5CB4E4]/10 border-[#5CB4E4]/30' : ''}`}>
                                {viewMode === 'list' ? (
                                    <div className="bg-white rounded-[3rem] border border-slate-200 shadow-xl overflow-hidden font-outfit">
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-[#041C3C] text-white">
                                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest italic">Student Name</th>
                                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest italic">Student ID</th>
                                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest italic">Status</th>
                                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest italic text-right">Attendance %</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100">
                                                {students.map((student: any) => (
                                                    <tr key={student.enrollmentId} className="hover:bg-slate-50 transition-all group">
                                                        ...
                                                        <td className="px-8 py-3">
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-2 h-2 rounded-full ${student.studentId ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-slate-300'}`} />
                                                                <span className="text-[10px] font-black text-[#041C3C] uppercase tracking-widest italic">
                                                                    {student.studentId ? 'Enrolled' : 'Not Registered'}
                                                                </span>
                                                            </div>
                                                        </td>
                                                        <td className="px-8 py-3 text-right">
                                                            <span className="text-base font-black text-[#041C3C] italic">{sessions.length > 0 ? Math.round((student.attendance.filter((a: any) => a.status === 'Present' || a.status === 'Late').length / sessions.length) * 100) : 0}%</span>
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        ...
                                        <div className="px-8 py-6 bg-slate-50 border-b border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div className="space-y-0.5">
                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Filter History</span>
                                                    <div className="relative">
                                                        <select 
                                                            value={filterDate ? (new Date(filterDate + 'T00:00:00').getMonth()).toString() : ''}
                                                            onChange={(e) => {
                                                                if (e.target.value === '') { setFilterDate(''); return; }
                                                                const currentYear = new Date().getFullYear();
                                                                const month = parseInt(e.target.value) + 1;
                                                                setFilterDate(`${currentYear}-${month.toString().padStart(2, '0')}-01`);
                                                            }}
                                            ...
                                            <table className="w-full text-left border-collapse">
                                                <thead>
                                                    <tr className="bg-slate-100/50">
                                                        <th className="px-8 py-4 text-[10px] font-black text-[#041C3C] uppercase tracking-widest border-b border-slate-200">Attendance Grid</th>
                                                        {filteredSessions.map((session: any) => (
                                                            <th key={session.id} className="px-6 py-4 border-b border-slate-200 min-w-[120px]">
                                                                <div className="flex flex-col items-center">
                                                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">
                                                                        {new Date(session.date).toLocaleDateString('en-US', { weekday: 'short' }).toUpperCase()}
                                                                    </span>
                                                                    <span className="text-sm font-black text-[#041C3C] uppercase italic leading-none">
                                                                        {new Date(session.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).toUpperCase()}
                                                                    </span>
                                                                </div>
                                                            </th>
                                                        ))}
                                                                        ...
                                                                        {isEditing ? (
                                                                            <select 
                                                                                ...
                                                                                className={`w-full bg-transparent text-center text-[10px] font-black uppercase tracking-widest cursor-pointer focus:outline-none transition-all rounded-lg py-1.5 ${
                                                                                    ...
                                                                                    status === 'Absent' ? 'text-rose-600 bg-rose-50' : 'text-sky-600 bg-sky-50'
                                                                                }`}
                                                                            >
                                                                            ...
                                                                            <span className={`text-[10px] font-black uppercase tracking-widest ${status === 'Present' ? 'text-emerald-600' : status === 'Absent' ? 'text-rose-600' : 'text-slate-400'}`}>
                                                                                {status.charAt(0)}
                                                                            </span>
                                                                        )}
                                                                    </td>
                    ...
                    <div className="bg-white border border-white/20 w-full max-w-xl rounded-[4rem] shadow-4xl p-16 animate-in zoom-in-95 duration-500 relative overflow-hidden">
                        ...
                        <div className="relative z-10 space-y-12">
                            <div className="text-center space-y-4">
                                ...
                                <h3 className="text-4xl font-black text-[#041C3C] uppercase tracking-tighter italic">EXCEPTION_ENROLLMENT</h3>
                                <div className="h-1 w-20 bg-[#5CB4E4]/30 rounded-full mx-auto" />
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic opacity-60">AUTHORIZING_MANUAL_STATUS_OVERRIDE</p>
                            </div>

                            <div className="space-y-6">
                                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner">
                                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300 mb-4 italic">SUBJECT_ID</p>
                                    <p className="text-2xl font-black text-[#041C3C] uppercase italic tracking-tight">{excuseTarget?.student?.studentName}</p>
                                </div>

                                <div className="space-y-4">
                                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 ml-4 italic">FORMAL_JUSTIFICATION</label>
                                    <textarea
                                        value={excuseReason}
                                        ...
                                        placeholder="INPUT_REASONING_PROTOCOL..."
                                    />
                                </div>

                                <div className="space-y-4">
                                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 ml-4 italic">OPTICAL_VERIFICATION_(OPTIONAL)</label>
                                    <label className="flex items-center gap-6 p-6 border-2 border-dashed border-slate-200 rounded-[2rem] cursor-pointer hover:border-[#5CB4E4] hover:bg-slate-50 transition-all group">
                                        ...
                                        <div className="flex-1 overflow-hidden">
                                            <p className="text-[10px] font-black text-[#041C3C] uppercase tracking-[0.2em] italic truncate">
                                                {excuseFile ? excuseFile.name : 'DEPOSIT_CREDENTIALS'}
                                            </p>
                                            <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mt-1 italic">LIMIT: 10_MB_JPG_PDF</p>
                                        </div>
                                        <input type="file" className="hidden" accept="image/*,.pdf" onChange={(e) => e.target.files && setExcuseFile(e.target.files[0])} />
                                    </label>
                                </div>
                            </div>

                            <div className="flex gap-6 pt-10 border-t border-slate-100">
                                <button onClick={() => setIsExcuseModalOpen(false)} className="flex-1 py-6 bg-white text-slate-400 font-black uppercase text-[11px] tracking-[0.3em] rounded-[2rem] border border-slate-200 hover:bg-slate-50 transition-all italic">ABORT</button>
                                <button
                                ...
                                >
                                    {isSubmittingExcuse ? 'PROCESSING...' : 'FINALIZE_OVERRIDE'} <CheckCircle size={18} className="group-hover:scale-125 transition-transform" />
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {isCancelModalOpen && (
                <div className="fixed inset-0 bg-rose-950/40 backdrop-blur-3xl z-[100] flex items-center justify-center p-8 animate-in fade-in duration-500">
                    <div className="bg-white border border-rose-100 w-full max-w-xl rounded-[4rem] shadow-4xl p-16 animate-in slide-in-from-bottom-10 duration-500 text-center">
                        ...
                        <h3 className="text-4xl font-black text-[#041C3C] uppercase tracking-tighter italic mb-6">SESSION_ARCHIVAL</h3>
                        <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] italic mb-12 opacity-60">CONFIRMING_PERMANENT_PROTOCOLS_SUSPENSION</p>
                        
                        <div className="space-y-8 text-left mb-12">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 ml-4 italic">ARCHIVAL_DATE</label>
                                <input
                                    type="date"
                                    value={cancelDate}
                            ...
                            <div className="space-y-4">
                                <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 ml-4 italic">FORMAL_ARCHIVE_REASON</label>
                                <textarea
                                    value={cancelReason}
                                    ...
                                    placeholder="INPUT_JUSTIFICATION..."
                                />
                            </div>
                        </div>

                        <div className="flex gap-6">
                            <button onClick={() => setIsCancelModalOpen(false)} className="flex-1 py-6 bg-white text-slate-400 font-black uppercase text-[11px] tracking-[0.3em] rounded-[2rem] border border-slate-200 hover:bg-slate-50 transition-all italic">ABORT</button>
                            <button
            ...
            {snapshotModal.isOpen && (
                <div className="fixed inset-0 bg-[#041C3C]/80 backdrop-blur-3xl z-[110] flex items-center justify-center p-8 animate-in fade-in duration-500" onClick={() => setSnapshotModal({ isOpen: false, url: '', studentName: '', date: '' })}>
                    <div className="bg-white/40 backdrop-blur-3xl w-full max-w-5xl rounded-[4.5rem] border border-white/20 shadow-4xl overflow-hidden animate-in zoom-in-95 duration-500" onClick={e => e.stopPropagation()}>
                        <div className="p-12 border-b border-white/20 flex justify-between items-center bg-white/60">
                            <div className="flex items-center gap-8">
                                ...
                                <div>
                                    <h4 className="text-3xl font-black text-[#041C3C] uppercase tracking-tighter italic leading-none mb-2">OPTICAL_EVIDENCE</h4>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic opacity-60">ID: {snapshotModal.studentName} | LOG: {snapshotModal.date}</p>
                                </div>
                            </div>
                            <button onClick={() => setSnapshotModal({ isOpen: false, url: '', studentName: '', date: '' })} className="w-14 h-14 flex items-center justify-center bg-white/60 hover:bg-[#041C3C] text-slate-400 hover:text-white rounded-2xl transition-all duration-500 border border-slate-200 active:scale-90">
                                <X size={24} />
                            </button>
                        </div>
                        <div className="p-16 flex items-center justify-center bg-slate-900 group">
                            <img
                                ...
                                onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/1200x800?text=OPTICAL_FEED_INTERRUPTED'; e.currentTarget.onerror = null; }}
                            />
                        </div>
                        <div className="p-10 text-center bg-white/60">
                            <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.8em] italic animate-pulse">ESTABLISHING_VISUAL_AUTHENTICITY...</span>
                        </div>
                    </div>
                </div>
            )}
