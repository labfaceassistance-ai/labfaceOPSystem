...
(async () => {
    try {
        ...
        console.error('[Analytics] Interventions table migration error:', err.message);
    }
})();
...
async function getProfessorClasses(professorPk) {
    const [classes] = await pool.query(
        `SELECT c.id, c.section, c.subject_code, c.academic_period_id
         FROM classes c
...
router.get('/professor/:userId/dashboard', authenticateToken, requireRole(['professor', 'admin']), async (req, res) => {
    try {
        ...
        const avgAttendance = rateRows[0].total > 0
            ? Math.round((rateRows[0].attended / rateRows[0].total) * 100)
            : 0;
        ...
        for (const s of studentStats) {
            const effAbs = (s.abs_count || 0) + Math.floor((s.late_count || 0) / 3);
...
router.get('/professor/:userId/absence-heatmap', authenticateToken, requireRole(['professor', 'admin']), async (req, res) => {
    try {
        ...
        const data = SLOTS.map(slot =>
            DAYS.map(day => {
                const matches = rows.filter(r => r.day_name === day && slot.hours.includes(Number(r.hour_val)));
