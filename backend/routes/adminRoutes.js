 ...
 * Returns high-level counts: total students/professors, today's attendance, active sessions
 */
router.get('/analytics/overview', authenticateToken, requireLabHead, async (req, res) => {
    try {
...
/**
 * GET /api/admin/analytics/trends?startDate=&endDate=&groupBy=day
 */
router.get('/analytics/trends', authenticateToken, requireLabHead, async (req, res) => {
    const { startDate, endDate, groupBy = 'day' } = req.query;
    try {
        const data = await analyticsService.getAttendanceTrends(startDate, endDate, groupBy);
        res.json(data);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
/**
 * GET /api/admin/analytics/students?limit=10
 */
router.get('/analytics/students', authenticateToken, requireLabHead, async (req, res) => {
    const limit = parseInt(req.query.limit) || 10;
    try {
        const data = await analyticsService.getStudentInsights(limit);
        res.json(data);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
/**
 * GET /api/admin/analytics/peak-times?startDate=&endDate=
 */
router.get('/analytics/peak-times', authenticateToken, requireLabHead, async (req, res) => {
    const { startDate, endDate } = req.query;
    try {
        const data = await analyticsService.getPeakUsageTimes(startDate, endDate);
        res.json(data);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
 ...
 * Returns DB / memory / CPU health + face recognition accuracy
 */
router.get('/health', authenticateToken, requireLabHead, async (req, res) => {
    try {
 ...
 * Returns in-memory performance metrics (no DB call)
 */
router.get('/performance', authenticateToken, requireLabHead, (req, res) => {
    try {
...
/**
 * POST /api/admin/reports/attendance
 * Generate PDF/Excel/CSV attendance report
 */
router.post('/reports/attendance', authenticateToken, requireLabHead, async (req, res) => {
    try {
 ...
 * Get list of reports generated by this admin user
 */
router.get('/reports', authenticateToken, requireLabHead, async (req, res) => {
    try {
